...
import ProjectBrowserDialog, { type ProjectLibraryEntry } from "../video-editor/ProjectBrowserDialog";
...
function IconButton({
  ...
  return (
    <button ref={buttonRef} type="button" className={`${styles.ib} ${styles.electronNoDrag} ${className}`} onClick={onClick} title={title}>
      {children}
    </button>
  );
}
...
function DropdownItem({
  ...
  return (
    <button type="button" className={`${styles.ddItem} ${selected ? styles.ddItemSelected : ""}`} onClick={onClick}>
...
function MicDeviceRow({ device, selected, onSelect }: { device: { deviceId: string; label: string }; selected: boolean; onSelect: () => void }) {
  ...
  return (
    <button type="button" className={`${styles.ddItem} ${selected ? styles.ddItemSelected : ""}`} onClick={onSelect}>
...
export function LaunchWindow() {
  ...
  const updateButtonLabel = updateStatus.status === "up-to-date" ? t("recording.update.updated") : t("recording.update.update");
  ...
  const idleControls = (
    <>
      ...
      <IconButton buttonRef={moreButtonRef} onClick={() => toggleDropdown("more")} title={t("recording.more")}>
            ...
            <div className={`${styles.menuCard} ${styles.electronNoDrag}`}>
                        ...
                        <>
                          ...
                          {windowSources.map((source) => (
                            <DropdownItem
                              ...
                              onClick={() => handleSourceSelect(source)}
                            >
                              {source.appName && source.appName !== source.name ? `${source.appName} — ${source.name}` : source.name}
                            </DropdownItem>
                          ))}
                        </>
                      )}
                      {screenSources.length === 0 && windowSources.length === 0 && (
                        <div className="text-center text-xs text-[#6b6b78] py-4">{t("recording.noSourcesFound")}</div>
                      )}
                    </>
                  )}
                </>
              ...
              {activeDropdown === "mic" && (
                <>
                  ...
                  {!microphoneEnabled && <div className="px-3 py-2 text-xs text-[#6b6b78]">{t("recording.selectMicToEnable")}</div>}
                  {devices.map((device) => (
                    <MicDeviceRow
                      key={device.deviceId}
                      device={device}
                      ...
                      onSelect={() => {
                        ...
                        setMicrophoneDeviceId(device.deviceId === "default" ? undefined : device.deviceId);
                      }}
                    />
                  ))}
                  {devices.length === 0 && <div className="text-center text-xs text-[#6b6b78] py-4">{t("recording.noMicrophonesFound")}</div>}
                </>
              )}
                ...
                <>
                  ...
                  {videoDevices.map((device) => (
                    <DropdownItem
                      key={device.deviceId}
                      icon={
                        webcamEnabled && (webcamDeviceId === device.deviceId || selectedVideoDeviceId === device.deviceId) ? (
                          <Video size={16} />
                        ) : (
                          <VideoOff size={16} />
                        )
                  ...
                  {videoDevices.length === 0 && <div className="text-center text-xs text-[#6b6b78] py-4">{t("recording.noWebcamsFound")}</div>}
                </>
              )}
              ...
              {activeDropdown === "more" && (
                <>
                  ...
                  <DropdownItem icon={<FolderOpen size={16} />} onClick={() => void openProjectBrowser()}>
